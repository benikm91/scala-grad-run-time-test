...
def main():
    ...
    for n_hidden_units in [8, 16, 32, 64, 128, 256, 512, 1024]:
        results = defaultdict(list)
        for experiment_id in range(0, 1):
            model = Net(n_hidden_units=n_hidden_units).to(DEVICE)
            # summary(model, (BATCH_SIZE, 28*28))
            ...
            for epoch in range(1, epochs + 1):
                ...
                results['elapsed_train_time'].append(elapsed_train_time)
                results['elapsed_test_time'].append(elapsed_test_time)
                results['test_acc'].append(test_acc)

        out_path = "out/results"
        # out_file = os.path.join(out_path, f"results_{n_hidden_units}.csv")
        out_file = os.path.join(out_path, f"pytorch_{n_hidden_units}.csv")
